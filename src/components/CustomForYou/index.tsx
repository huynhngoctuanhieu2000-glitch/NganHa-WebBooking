...
export default function CustomForYouModal({
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-0 sm:p-6 animate-in fade-in duration-200">
            ...
            {/* Modal Content - Better mobile height to prevent header cutoff */}
            <div className="relative w-full sm:w-[95vw] max-w-2xl bg-white rounded-t-[32px] rounded-b-none sm:rounded-[32px] shadow-2xl overflow-hidden flex flex-col h-[85vh] sm:h-[80vh] animate-in slide-in-from-bottom-10 sm:zoom-in-95 duration-300">
                ...
                {/* Footer Action */}
                <div className="border-t border-gray-100 bg-gray-50 pb-[env(safe-area-inset-bottom)] z-20 shrink-0">
                    <button
                        onClick={() => onSave(prefs)}
                        className="w-full h-[64px] shrink-0 bg-[#1a1c2e] hover:bg-[#2e314a] text-white font-bold rounded-none sm:rounded-b-[32px] flex items-center justify-center gap-2 text-base transition-all active:scale-[0.98] shadow-lg shadow-black/10"
                    >
                        <Check size={20} />
