...
// 🔧 UI CONFIGURATION
// ─────────────────────────────────────────────────────────
// Tất cả giá trị dưới đây đều dùng đơn vị px (không dùng rem/vh)
// để đảm bảo giao diện nhất quán trên mọi thiết bị và DPI.
// ─────────────────────────────────────────────────────────
const CONFIG = {
    // 🎞️ Animation
    ANIMATION_DURATION: 300,              // ms - thời gian slide-in/out

    // 📐 Layout
    BORDER_RADIUS: '30px',               // bo góc trên của sheet
    MAX_HEIGHT: '700px',                 // chiều cao tối đa của sheet (không co theo vh)
    HEADER_IMAGE_HEIGHT: '192px',        // chiều cao ảnh header = 12rem = 192px

    // 🎨 Màu sắc (đồng bộ 100% với wrb-noi-bo-dev)
    OVERLAY_COLOR: 'bg-black/60',        // lớp tối phủ phía sau sheet
    BG_COLOR: 'bg-[#1e293b]',           // nền chính của sheet (navy đậm - đã sửa từ #111111)
    FOOTER_BG: 'bg-[#1e293b]',          // nền footer (đồng nhất với sheet)
    GRADIENT_FROM: '#1e293b',           // màu bắt đầu của gradient che header image

    // 🔘 Nút bấm
    BUTTON_HEIGHT: '52px',              // chiều cao cố định nút CTA (không co giãn theo chữ)
};
...
const TEXT = {
    ...
    back_to_list: { vi: 'Xem danh sách', en: 'Back to List', cn: '回到列表', jp: 'リストに戻る', kr: '목록으로' },
    view_more: { vi: 'Xem thêm', en: 'View More', cn: '查看更多', jp: 'もっと見る', kr: '더 보기' },
    choose_duration: { vi: 'CHỌN THỜI GIAN', en: 'CHOOSE DURATION', cn: '选择时长', jp: '時間を選択', kr: '시간 선택' },
    update_cart: { vi: 'CẬP NHẬT GIỎ HÀNG', en: 'UPDATE CART', cn: '更新购物车', jp: 'カートを更新', kr: '장바구니 업데이트' },
...
export default function MainSheet({ group, cart, isOpen, lang, onClose, onAddToCart }: MainSheetProps) {
    ...
    return (
        <>
            <div className={`fixed inset-0 ${CONFIG.OVERLAY_COLOR} z-[55] transition-opacity duration-${CONFIG.ANIMATION_DURATION} ${isClosing ? 'opacity-0' : 'opacity-100'}`} onClick={handleClose} />

            {/* Sheet Container */}
            {/* CRITICAL: overflow-hidden là chì khóa nạn flex-1 expand theo maxHeight.
                Không có nó, flex-1 sẽ căng ra 700px dù chỉ có 1 item => gap trống khổng lồ. */}
            <div
                className={`fixed bottom-0 left-0 w-full ${CONFIG.BG_COLOR} overflow-hidden flex flex-col shadow-2xl z-[60]
                ...
                */}
                ...
                {viewMode === 'ADD' && selectedService && (
                    <div className="w-full relative shrink-0" style={{ height: CONFIG.HEADER_IMAGE_HEIGHT }}>
                        ...
                        {/* Gradient màu đồng nhất với BG_COLOR để blend mượt vào nền sheet */}
                        <div className="absolute inset-0" style={{ background: `linear-gradient(to top, ${CONFIG.GRADIENT_FROM}, transparent)` }}></div>
                        <div className="absolute bottom-4 left-5 right-5">
                            <h2 className="text-2xl font-bold text-white font-luxury leading-tight">{groupName}</h2>
                            {/* Mô tả dịch vụ - đồng bộ từ wrb-noi-bo-dev */}
                ...
                {/* Scrollable Content — không cần min-h-0 khi sheet đã có overflow-hidden */}
                        ...
                        <div className="px-5 mt-2">
                            ...
                            <div className="mb-6">
                                <div className="flex items-center justify-between mb-3">
                                    <h3 className="text-xs font-bold text-gray-500 uppercase tracking-widest">{t('choose_duration')}</h3>
                                    ...
                                    {group
                                        ...
                                        .map((svc) => (
                                            ...
                                            >
                                                ...
                                                {/* Guard: chỉ hiện phút khi timeValue > 0 - đồng bộ wrb-noi-bo-dev */}
                                                {svc.timeValue > 0 && (
                                                ...
                                                {/* Badge số lượng trong giỏ khi đang chọn card khác - đồng bộ wrb-noi-bo-dev */}
                ...
                {/* FOOTER ACTION */}
                {viewMode === 'ADD' && selectedService && (
                    <div className={`p-5 pt-2 ${CONFIG.FOOTER_BG} border-t border-gray-700/50`}>
                        {/* mb-6 = 24px — khớp y chang wrb-noi-bo-dev */}
                        <div className="flex items-center justify-center mb-6">
                            <div className="flex items-center gap-6 bg-gray-800 rounded-full p-2 border border-gray-700 px-6">
                                <button onClick={() => setQty(q => Math.max(1, q - 1))} className="w-10 h-10 rounded-full bg-gray-700 text-white flex items-center justify-center hover:bg-gray-600 transition-colors">
                                    <Minus size={18} />
                                </button>
                                <span className="text-xl font-bold text-white min-w-[30px] text-center font-mono">{qty}</span>
                                <button onClick={() => setQty(q => q + 1)} className="w-10 h-10 rounded-full bg-yellow-500 text-black flex items-center justify-center hover:bg-yellow-400 transition-colors">
                                    <Plus size={18} />
                                </button>
                            </div>
                        </div>

                        {/* py-3.5 — khớp y chang wrb-noi-bo-dev, không dùng fixed height */}
                        <button
                            onClick={handleConfirm}
                            className="w-full h-[52px] shrink-0 bg-gradient-to-r from-yellow-600 to-yellow-500 text-black font-bold rounded-xl shadow-lg flex items-center justify-center gap-2 text-base uppercase hover:brightness-110 transition-all overflow-hidden"
                        >
                            <span className="whitespace-nowrap">{cart[selectedService.id] ? t('update_cart') : t('add_to_cart')}</span>
                            <span className="opacity-40 shrink-0">|</span>
                            <span className="whitespace-nowrap">{formatCurrency(selectedService.priceVND * qty)} VND</span>
                            <span className="opacity-40 shrink-0">/</span>
                            <span className="text-red-600 font-bold whitespace-nowrap">{selectedService.priceUSD * qty} USD</span>
                        </button>
                    </div>
                )}
            </div>
